...
(global-set-key (kbd "C-M-j") 'beginning-of-buffer)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (defhydra hydra-breadcrumbs (:exit nil :hint nil)
     ...
     _k_ → Prev            _J_ → File Prev      _cc_ → Clear
  ————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
...
#+BEGIN_SRC emacs-lisp
  ...
  (defhydra hydra-super-menu2 (:exit t :foreign-keys nil :hint nil)
    ...
    _bu_ → Buffers        _bm_ → Boockmarks  _mc_ → Multi-cursor
  ...
  (defhydra hydra-super-menu (:exit t :foreign-keys nil :hint nil)
    ...
    _w_  → Ace window     _j_  → Jump char      _g_  → Ag grep          _c_  → Config emacs
  ————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
